...
QPot::QPot(QWidget *parent) : QWidget(parent)
{
    spanAngle = 0;
    startAngle = 225 * 16;
    startPoint = 225 * 16;
...
void QPot::paintEvent(QPaintEvent *)
{
    paintBorder();
    paintArc();
    paintDial();
...
void QPot::setMinValue(double minValue)
{
    min = minValue;
    update();
}

void QPot::setMaxValue(double maxValue)
{
    max = maxValue;
    update();
}

void QPot::setDotStep(int dotStep)
{
    dot = dotStep;
    update();
}
void QPot::setWheelStep(int wheelStep)
{
    wheel = wheelStep;
    update();
}

void QPot::setFontDim(int fontDim)
{
    font = fontDim;
    update();
}

void QPot::setPrecision(int numPrec)
{
    precision = numPrec;
    update();
}

void QPot::setFstColor(MeterColor color)
{
    fstColor = color;
    update();
}

void QPot::setSndColor(MeterColor color)
{
    sndColor = color;
    update();
}

void QPot::wheelEvent(QWheelEvent *e)
{
...
QSize QPot::minimumSizeHint() const
{
    return QSize(20, 20);
}

QSize QPot::sizeHint() const
{
